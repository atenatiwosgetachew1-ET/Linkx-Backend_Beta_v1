...
# 1. Replace the get_smurfing_query definition
new_smurfing_gen = '''def get_smurfing_query(label, scope_clause_t, trusted_pair_clause, is_provisional=False, incremental_batch_id=None):
...
# Extract the old get_smurfing_query block and replace it
# It starts at "def get_smurfing_query" and ends before "def get_circular_flow_query"
pattern_gen = r"def get_smurfing_query.*?def get_circular_flow_query"
text = re.sub(pattern_gen, new_smurfing_gen + "\n\ndef get_circular_flow_query", text, flags=re.DOTALL)
