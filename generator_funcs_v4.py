...
def get_shared_identifier_query(label, scope_clause_t, is_provisional=False, incremental_batch_id=None):
    prov_str = "true" if is_provisional else "false"
    # For shared identifier, incremental logic is incredibly complex in cypher, but basically we get phones from the seed.
    seed_block = ""
    match_filters = ""
    if incremental_batch_id:
        seed_block = f"""
    ...
    WHERE ({scope_clause_t})
      AND coalesce(t.IGNORE_LOGICAL, false) = false
      {match_filters}
    WITH t, {{"identifier_type, identifier_value" if incremental_batch_id else "[{kind:'BUSINESSMOBILENO', value:t.BUSINESSMOBILENO, account:t.LOGICAL_ACCOUNTNO}, {kind:'BENTELNO', value:t.BENTELNO, account:t.LOGICAL_BENACCOUNTNO}] AS identifiers"}}
    {"WITH identifier_type, identifier_value, CASE WHEN t.BUSINESSMOBILENO = identifier_value THEN t.LOGICAL_ACCOUNTNO ELSE t.LOGICAL_BENACCOUNTNO END AS account, t" if incremental_batch_id else "UNWIND identifiers AS identifier WITH identifier.kind AS identifier_type, trim(toString(identifier.value)) AS identifier_value, identifier.account AS account, t"}
...
def get_rapid_withdrawal_query(label, scope_clause_t, is_provisional=False, incremental_batch_id=None):
    ...
    WHERE ({scope_clause_t})
      AND coalesce(t.IGNORE_LOGICAL, false) = false
      {match_filters if incremental_batch_id else "AND t.TRANSACTIONDATE IS NOT NULL AND t.TRANSACTIONDATE <> '' AND t.LOGICAL_ACCOUNTNO IS NOT NULL AND t.LOGICAL_ACCOUNTNO <> '' AND NOT t.LOGICAL_ACCOUNTNO IN $pt"}
    WITH {"acc, tx_day," if incremental_batch_id else "t.LOGICAL_ACCOUNTNO AS acc, t.TRANSACTIONDATE AS tx_day,"} collect(t) AS txns
...
def get_account_activity_spike_query(label, scope_clause_t, is_provisional=False, incremental_batch_id=None):
    ...
    WHERE ({scope_clause_t})
      AND coalesce(t.IGNORE_LOGICAL, false) = false
      {match_filters if incremental_batch_id else "AND t.TRANSACTIONDATE IS NOT NULL AND t.TRANSACTIONDATE <> '' AND t.LOGICAL_ACCOUNTNO IS NOT NULL AND t.LOGICAL_ACCOUNTNO <> '' AND NOT t.LOGICAL_ACCOUNTNO IN $pt"}
    WITH {"acc, tx_day," if incremental_batch_id else "t.LOGICAL_ACCOUNTNO AS acc, t.TRANSACTIONDATE AS tx_day,"} count(t) AS daily_count, collect(t) AS txns
