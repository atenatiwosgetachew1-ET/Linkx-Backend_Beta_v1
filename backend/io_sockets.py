    ...
    def handle_graph_status_subscribe(data):
        ...
        if session_id in entry.get("graph_statuses", {}):
            ...
            if is_socket_alive(sid):
                # Socket still alive → do not start another task
                print(f"Graph status already running for session {session_id}, sid {sid}")
                return
            else:
                # Socket is dead → clean up stale task before starting a new one
                print(f"Cleaning up stale graph status for session {session_id}, sid {sid}")
                # stop the background task if it's still running
                if g_entry.get("task") and g_entry["task"].is_alive():
                    g_entry["stop_event"].set()  # signal the thread to stop
