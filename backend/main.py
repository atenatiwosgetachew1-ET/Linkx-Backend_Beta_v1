...
def init():
    ...
    try:
        ...
        configs={
            ...
            "hive_port":"10000",
            "api_port":"5000",
            "search_api_endpoint":"api/search",
            "search_columns":["transactionid","businessmobileno","accountno","benaccountno","bentelno"],
...
def live_batch_files():
    ...
    if action_id == "create_DF":
        ...
        # -----------------------------
        # 2) Align & merge once, based on type
        # -----------------------------
        # Collect all columns
        all_columns = set()
        for df in dfs:
        ...
        try:
            if isinstance(first_df, pd.DataFrame):
                merged = merge_pandas_and_save(aligned, path_to_save, session_id)
                num_rows = len(merged)
                columns = list(merged.columns)
        
            elif 'pyspark.sql.dataframe.DataFrame' in str(type(first_df)):
                merged = merge_spark_and_save(aligned, path_to_save, session_id)
                num_rows = merged.count()
                columns = merged.columns
        
            else:
                return jsonify({
                    "results": "",
                    "message": f"Unsupported DataFrame type: {type(first_df)}"
                }), 400
        
            if merged is None:
                return jsonify({
                    "results": "",
                    "message": "Failed to create DF!"
                }), 400
    ...
    # -----------------------------
        ...
        df = data.get("df")
        if df is None:
            return jsonify({'results': None, 'message': 'No dataframe provided for analysis'}), 400

        payload = {"id": "analyze", "df": df, "session_id": session_id}
        result = batch_data_manager(payload)
        return jsonify({'results': result, 'message': 'Analysis completed!'}), 200
