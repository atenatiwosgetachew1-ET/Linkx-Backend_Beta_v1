...
def main(driver, nodes_label, log_file, high_risk_accounts=None, threshold_multiplier=3):
    ...
    with driver.session() as session:
        ...
        session.run("""
            ...
            MERGE (a)-[r1:CIRCULAR_FLOW]->(b)
            SET r1.bgcolor = '#e6e6e6'
            MERGE (b)-[r2:CIRCULAR_FLOW]->(a)
        ...
        session.run("""
            ...
            MERGE (current_txn)-[r:HIGH_RISK_LINK]->(prev_txn)
            SET r.bgcolor = '#ba720d'
        """, nodes_label=nodes_label, accounts=high_risk_accounts)
        ...
        session.run("""
            ...
            WITH current,
                 [tx IN prev_txns WHERE tx.BALANCE IS NOT NULL |
                     abs(current.BALANCE - tx.BALANCE)
                 ] AS changes,
                 prev_txns
            WITH current,
                 changes,
            ...
            MERGE (prev_tx)-[r:ABNORMAL_BALANCE_CHANGE]->(current)
            SET r.bgcolor = '#196e08',
                r.change = abs(current.BALANCE - prev_tx.BALANCE)
        """, nodes_label=nodes_label, threshold=threshold_multiplier)
