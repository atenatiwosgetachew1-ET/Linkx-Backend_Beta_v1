...
def neo4j_row_data_injector(payload, batch_size=500):
    ...
    try:
        ...
        if action == "Source / Target Relationship":
            ...
            # Function to sanitize row properties (remove None and illegal keys)
            def sanitize_props(d):
                return {k.replace(" ", "_").replace(".", "_"): v for k, v in d.items() if v is not None}

            # Select full rows but distinct on source/target
            rows = (
            ...
            for r in rows:
                ...
                rels.append({
                    "source": r[source_col],
                    "target": r[target_col],
                    "props": row_dict   # sanitized properties
                })
            ...
            with driver.session() as session:
                ...
                # Remove previous relationships for this session
                session.run("""
                    MATCH ()-[r]->()
                ...
                for i in range(0, total_rels, batch_size):
                    ...
                    session.run(f"""
                        ...
                        ON CREATE SET b += row.props

                        MERGE (a)-[rel:{relationship_type}]->(b)
                        ON CREATE SET rel.bgcolor = '#750b8c',
