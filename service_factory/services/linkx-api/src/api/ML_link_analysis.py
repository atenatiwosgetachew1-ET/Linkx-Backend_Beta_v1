...
ML_link_analysis_api = Blueprint("ML_link_analysis_api", __name__)
...
def _ingest_dataframe_to_neo4j(session_id, entity):
    ...
    if not _run_analyzer(link_payload, "ml link analysis"):
        return False
    ...
    if source_target:
        ...
        if not _run_analyzer(relationship_payload, "ml source target relationship"):
            return False
...
@ML_link_analysis_api.route("/ML_link_analysis", methods=["POST"])
@ML_link_analysis_api.route("/ml_service/link_analysis", methods=["POST"])
@validate_json_payload(COMMON_SCHEMAS["ml_link_analysis"], error_message="failed!", include_detail=False)
def ML_link_analysis():
    ml_api_key = os.getenv("LINKX_ML_SERVICE_API_KEY") or os.getenv("LINKX_PUBLIC_API_KEY")
    if ml_api_key and request.headers.get("X-API-Key") != ml_api_key:
    ...
    # Ensure session ID uses the required 'ml_service' prefix
    raw_id = str(data.get("ml_id") or data.get("session_id") or "")
    if raw_id:
        session_id = raw_id if raw_id.startswith("ml_service") else f"ml_service_{raw_id}"
    else:
        session_id = f"ml_service_{uuid4().hex}"
