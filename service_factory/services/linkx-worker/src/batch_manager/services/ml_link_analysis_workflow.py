...
def _run_analyzer(payload, step_name):
    print(f"ML link analysis {step_name} analyzer payload:", redact_value(payload), flush=True)
    try:
        return analyzer(payload) is True
    except Exception as exc:
        print(f"ML link analysis {step_name} analyzer failed:", exc)
        return False
...
def _ingest_dataframe_to_neo4j(session_id, entity):
    ...
    if not credentials:
        print("ML link analysis missing neo4j credentials")
        return False
...
def run_ml_link_analysis(data):
    ...
    if entity != "bank" or type_value != "account_number":
        return {"message": "failed", "status": "failed", "detail": "unsupported_ml_request", "session_id": session_id}
    ...
    print("ML link analysis worker request", {"session_id": session_id, "entity": entity, "type": type_value, "elastic_api_url": api_url})
