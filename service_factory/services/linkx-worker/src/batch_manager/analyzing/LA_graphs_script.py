...
def prepare_graph_data_full(records):
    ...
    for rec in records:
        ...
        n_id = str(a_props.get("NodeId") or getattr(a_node, 'element_id', 'unknown_n'))
        m_id = str(b_props.get("NodeId") or getattr(b_node, 'element_id', 'unknown_m'))
        r_id = str(getattr(rel, 'element_id', 'unknown_r'))
        ...
        if n_id not in nodes_dict:
            nodes_dict[n_id] = {
                'id': n_id,
                'label': a_props.get("NodeId", n_id),
                'title': build_node_properties_full(a_node),
                **a_props
            }
        ...
        if m_id not in nodes_dict:
            nodes_dict[m_id] = {
                'id': m_id,
                'label': b_props.get("NodeId", m_id),
                'title': build_node_properties_full(b_node),
                **b_props
            }
            ...
            with driver.session() as session:
                while True:
                    ...
                    for record in session.run(
                        ...
                        n_id = str(a_props.get("NodeId") or getattr(a, 'element_id', a.id))
                        m_id = str(b_props.get("NodeId") or getattr(b, 'element_id', b.id))
                        ...
                        if chunk_callback:
                            node_ids.add(n_id)
                            node_ids.add(m_id)
                            if n_id not in emitted_node_ids:
                                chunk_nodes[n_id] = a_node
                                emitted_node_ids.add(n_id)
                            if m_id not in emitted_node_ids:
                                chunk_nodes[m_id] = b_node
                                emitted_node_ids.add(m_id)
