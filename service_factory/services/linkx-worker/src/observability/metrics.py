...
def start_otel_metrics_server(default_port=8889):
    ...
    try:
        ...
        print(f"OpenTelemetry worker metrics server listening on port {port}", flush=True)
        return True
    except Exception as exc:
        print(f"Failed to start OpenTelemetry worker metrics server on port {port}: {exc}", flush=True)
        return False
