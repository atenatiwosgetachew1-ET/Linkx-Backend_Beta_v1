def get_smurfing_query(label, scope_clause, trusted_pair_clause, is_provisional=False):
...
def get_circular_flow_query(label, scope_clause_t, scope_clause_a, scope_clause_b, trusted_pair_clause, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
...
def get_dormant_to_active_query(label, scope_clause_t, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_abnormal_balance_query(label, scope_clause_t, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_hub_and_spoke_out_query(label, scope_clause_t, trusted_pair_clause, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_hub_and_spoke_in_query(label, scope_clause_t, trusted_pair_clause, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_shared_identifier_query(label, scope_clause_t, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_rapid_withdrawal_query(label, scope_clause_t, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_account_activity_spike_query(label, scope_clause_t, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
...
def get_high_risk_link_query(label, scope_clause_t, is_provisional=False):
    prov_str = "true" if is_provisional else "false"
    return f"""
    MATCH (t:{label})
    WHERE ({scope_clause_t})
